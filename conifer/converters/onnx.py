...
def convert_graph(onnx_clf):
  ...
  children_right=np.array(node.attribute[get_key('nodes_falsenodeids')].ints)
  children_left=np.array(node.attribute[get_key('nodes_truenodeids')].ints)
  threshold=np.array(node.attribute[get_key('nodes_values')].floats)
  feature=np.array(node.attribute[get_key('nodes_featureids')].ints)
  values=np.array(node.attribute[get_key('class_weights')].floats)
  modes=np.array(node.attribute[get_key('nodes_modes')].strings)
